...
from machine_learning.tasks import batch_users_prediction_task
...
def rate_movie_view(request):
    ...
    if user.is_authenticated:
        ...
        if rating_obj.content_object is not None:
            ...
            start_page = request.session.get('total-new-suggestions') or 0
            if items_rated % 5 ==0:               
                batch_users_prediction_task.apply_async(kwargs={'user_ids': [user.id], 'start_page':start_page, 'max_pages':10})       
